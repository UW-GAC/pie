...
class GlobalStudyFactoryTestCase(TestCase):
    ...
    def test_global_study_factory_build_batch(self):
        """A GlobalStudy instance is returned by GlobalStudyFactory.build_batch(5)."""
        global_studies = factories.GlobalStudyFactory.build_batch(5)
        for one in global_studies:
            self.assertIsInstance(one, models.GlobalStudy)

    def test_global_study_factory_create_batch(self):
        """A GlobalStudy instance is returned by GlobalStudyFactory.create_batch(5)."""
        global_studies = factories.GlobalStudyFactory.create_batch(5)
        for one in global_studies:
            self.assertIsInstance(one, models.GlobalStudy)
...
class StudyFactoryTestCase(TestCase):
    ...
    def test_study_factory_build_batch(self):
        """A Study instance is returned by StudyFactory.build_batch(5)."""
        studies = factories.StudyFactory.build_batch(5)
        for one in studies:
            self.assertIsInstance(one, models.Study)

    def test_study_factory_create_batch(self):
        """A Study instance is returned by StudyFactory.create_batch(5)."""
...
class SourceStudyVersionFactoryTestCase(TestCase):
    ...
    def test_source_study_version_factory_build_batch(self):
        """A SourceStudyVersion instance is returned by SourceStudyVersionFactory.build_batch(5)."""
        source_study_versions = factories.SourceStudyVersionFactory.build_batch(5)
    ...
    def test_source_study_version_factory_create_batch(self):
        """A SourceStudyVersion instance is returned by SourceStudyVersionFactory.create_batch(5)."""
        source_study_versions = factories.SourceStudyVersionFactory.create_batch(5)
        for one in source_study_versions:
            self.assertIsInstance(one, models.SourceStudyVersion)
...
class SubcohortFactoryTestCase(TestCase):
    ...
    def test_source_trait_factory_build_batch(self):
        """A Subcohort instance is returned by SubcohortFactory.build_batch(5)."""
        source_traits = factories.SubcohortFactory.build_batch(5)
        for one in source_traits:
            self.assertIsInstance(one, models.Subcohort)

    def test_source_trait_factory_create_batch(self):
        """A Subcohort instance is returned by SubcohortFactory.create_batch(5)."""
        source_traits = factories.SubcohortFactory.create_batch(5)
        for one in source_traits:
            self.assertIsInstance(one, models.Subcohort)
...
class SourceDatasetFactoryTestCase(TestCase):
    ...
    def test_source_dataset_factory_build_batch(self):
        """A SourceDataset instance is returned by SourceDatasetFactory.build_batch(5)."""
        source_datasets = factories.SourceDatasetFactory.build_batch(5)
    ...
    def test_source_dataset_factory_create_batch(self):
        """A SourceDataset instance is returned by SourceDatasetFactory.create_batch(5)."""
        source_datasets = factories.SourceDatasetFactory.create_batch(5)
        for one in source_datasets:
            self.assertIsInstance(one, models.SourceDataset)
...
class HarmonizedTraitSetFactoryTestCase(TestCase):
    ...
    def test_harmonized_trait_set_factory_build_batch(self):
        """A HarmonizedTraitSet instance is returned by HarmonizedTraitSetFactory.build_batch(5)."""
        harmonized_trait_sets = factories.HarmonizedTraitSetFactory.build_batch(5)
    ...
    def test_harmonized_trait_set_factory_create_batch(self):
        """A HarmonizedTraitSet instance is returned by HarmonizedTraitSetFactory.create_batch(5)."""
        harmonized_trait_sets = factories.HarmonizedTraitSetFactory.create_batch(5)
        for one in harmonized_trait_sets:
            self.assertIsInstance(one, models.HarmonizedTraitSet)
...
class HarmonizationUnitFactoryTestCase(TestCase):
    ...
    def test_build_batch(self):
        """A HarmonizationUnit instance is returned by HarmonizationUnitFactory.build_batch(5)."""
        harmonization_units = factories.HarmonizationUnitFactory.build_batch(5)
        for one in harmonization_units:
            self.assertIsInstance(one, models.HarmonizationUnit)

    def test_create_batch(self):
        """A HarmonizationUnit instance is returned by HarmonizationUnitFactory.create_batch(5)."""
        harmonization_units = factories.HarmonizationUnitFactory.create_batch(5)
        for one in harmonization_units:
            self.assertIsInstance(one, models.HarmonizationUnit)
    ...
    def test_create_with_component_harmonized_trait_sets(self):
        ...
        harmonized_trait_sets = factories.HarmonizedTraitSetFactory.create_batch(10)
        harmonization_unit = factories.HarmonizationUnitFactory.create(
            component_harmonized_trait_sets=harmonized_trait_sets)
    ...
    def test_create_with_component_source_and_harmonized_and_batch_traits(self):
        ...
        source_traits = factories.SourceTraitFactory.create_batch(5)
        batch_traits = factories.SourceTraitFactory.create_batch(5)
        harmonized_trait_sets = factories.HarmonizedTraitSetFactory.create_batch(10)
        harmonization_unit = factories.HarmonizationUnitFactory.create(
...
class SourceTraitFactoryTestCase(TestCase):
    ...
    def test_source_trait_factory_build_batch(self):
        """A SourceTrait instance is returned by SourceTraitFactory.build_batch(5)."""
        source_traits = factories.SourceTraitFactory.build_batch(5)
        for one in source_traits:
            self.assertIsInstance(one, models.SourceTrait)

    def test_source_trait_factory_create_batch(self):
        """A SourceTrait instance is returned by SourceTraitFactory.create_batch(5)."""
        source_traits = factories.SourceTraitFactory.create_batch(5)
        for one in source_traits:
            self.assertIsInstance(one, models.SourceTrait)
...
class HarmonizedTraitFactoryTestCase(TestCase):
    ...
    def test_build_batch(self):
        """A HarmonizedTrait instance is returned by HarmonizedTraitFactory.build_batch(5)."""
        harmonized_traits = factories.HarmonizedTraitFactory.build_batch(5)
        for one in harmonized_traits:
            self.assertIsInstance(one, models.HarmonizedTrait)

    def test_create_batch(self):
        """A HarmonizedTrait instance is returned by HarmonizedTraitFactory.create_batch(5)."""
        harmonized_traits = factories.HarmonizedTraitFactory.create_batch(5)
        for one in harmonized_traits:
            self.assertIsInstance(one, models.HarmonizedTrait)
...
class SourceTraitEncodedValueFactoryTestCase(TestCase):
    ...
    def test_source_trait_encoded_value_factory_build_batch(self):
        """A SourceTraitEncodedValue instance is returned by SourceTraitEncodedValueFactory.build_batch(5)."""
        source_trait_encoded_values = factories.SourceTraitEncodedValueFactory.build_batch(5)
    ...
    def test_source_trait_encoded_value_factory_create_batch(self):
        """A SourceTraitEncodedValue instance is returned by SourceTraitEncodedValueFactory.create_batch(5)."""
        source_trait_encoded_values = factories.SourceTraitEncodedValueFactory.create_batch(5)
        for one in source_trait_encoded_values:
            self.assertIsInstance(one, models.SourceTraitEncodedValue)
...
class HarmonizedTraitEncodedValueFactoryTestCase(TestCase):
    ...
    def test_harmonized_trait_encoded_value_factory_build_batch(self):
        """A HarmonizedTraitEncodedValue instance is returned by HarmonizedTraitEncodedValueFactory.build_batch(5)."""
        harmonized_trait_encoded_values = factories.HarmonizedTraitEncodedValueFactory.build_batch(5)
    ...
    def test_harmonized_trait_encoded_value_factory_create_batch(self):
        """A HarmonizedTraitEncodedValue instance is returned by HarmonizedTraitEncodedValueFactory.create_batch(5)."""
        harmonized_trait_encoded_values = factories.HarmonizedTraitEncodedValueFactory.create_batch(5)
        for one in harmonized_trait_encoded_values:
            self.assertIsInstance(one, models.HarmonizedTraitEncodedValue)
