...
def profile(request):
    ...
    savedsource = SourceSearchTable(build_usersearches(request.user.id, 'source'))
    savedharmonized = HarmonizedSearchTable(build_usersearches(request.user.id, 'harmonized'))
